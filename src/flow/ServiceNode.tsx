...
// AWS SMILE LOGO SVG
const AWSLogo = () => (
  <div className="flex flex-col items-end select-none opacity-90 shrink-0 select-none">
    <span className="text-[10px] font-black tracking-tighter text-white mr-2 select-none">aws</span>
    <svg className="w-9 h-2.5 text-amber-500 select-none" viewBox="0 0 35 10" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export const ServiceNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  const getBrandLogo = (serviceName: string) => {
    ...
    return (
      <div className="w-8 h-8 rounded-lg bg-indigo-950/40 border border-indigo-500/20 flex items-center justify-center text-indigo-400 shrink-0">
        <Cpu className="h-4 w-4 animate-pulse" />
      </div>
    );
  };

  // Status visual styles (Success = green, Error = red)
  const statusStyles = {
    healthy: {
  ...
  return (
    <div
      className={`w-80 rounded-2xl bg-[#090a0d] border shadow-2xl transition-all duration-300 overflow-hidden text-neutral-200 select-none ${
        selected
          ? 'border-indigo-500 ring-2 ring-indigo-500/30 shadow-indigo-600/10 scale-[1.02]'
          : 'border-neutral-850 border-neutral-800/80 hover:border-neutral-700'
      }`}
    >
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            {getBrandLogo(name)}
            <h4 className="text-sm font-semibold text-white truncate max-w-[120px]" title={name}>
              {name || 'Service'}
            </h4>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button className="p-1.5 rounded-md hover:bg-neutral-800 text-neutral-500 hover:text-neutral-200 transition-colors">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="grid grid-cols-4 gap-1">
            ...
            {/* Memory Inactive */}
            <div className="bg-neutral-800/40 border border-neutral-800/80 text-neutral-400 rounded flex items-center justify-center gap-1 py-1 font-medium text-[9px]">
